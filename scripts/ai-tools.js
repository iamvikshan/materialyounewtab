...
const aiTools = aiToolsRaw.map(tool => ({
  ...tool,
  label:
    translations[currentLanguage]?.[tool.id] || translations['en'][tool.id],
}));
...
const aiToolsSettingsOverlay = document.getElementById(
  'aiToolsSettingsOverlay'
);
...
function applyAIToolsSettings() {
  const savedSettings = JSON.parse(
    localStorage.getItem('aiToolsSettings') || 'null'
  );
  ...
  toolOptions.forEach((option, index) => {
    ...
    if (downButton) {
      ...
      downButton.style.visibility =
        index === toolOptions.length - 1 ? 'hidden' : 'visible';
    }
  });
}
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.addEventListener('click', event => {
    if (
      !aiToolName.contains(event.target) &&
      aiToolName.style.display === 'flex'
    ) {
      toggleAITools();
    }
