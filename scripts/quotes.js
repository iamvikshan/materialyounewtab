...
async function fetchAndDisplayQuote() {
  try {
    ...
    if (
      quote.length + author.length >= MIN_QUOTE_LENGTH &&
      quote.length + author.length <= MAX_QUOTE_LENGTH
    ) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const motivationalQuotesCont = document.getElementById(
    'motivationalQuotesCont'
  );
  const motivationalQuotesCheckbox = document.getElementById(
    'motivationalQuotesCheckbox'
  );
  const searchWithContainer = document.getElementById('search-with-container');

  // Load states from localStorage
  hideSearchWith.checked =
    localStorage.getItem('showShortcutSwitch') === 'true';
  motivationalQuotesCheckbox.checked =
    localStorage.getItem('motivationalQuotesVisible') !== 'false';
  ...
  const updateMotivationalQuotesState = () => {
    ...
    searchWithContainer.style.display = isMotivationalQuotesEnabled
      ? 'none'
      : 'flex';
    motivationalQuotesCont.style.display = isMotivationalQuotesEnabled
      ? 'flex'
      : 'none';
  ...
  motivationalQuotesCheckbox.addEventListener(
    'change',
    updateMotivationalQuotesState
  );
});
