...
document.addEventListener('DOMContentLoaded', function () {
  ...
  bookmarkSearch.addEventListener('input', function () {
    ...
    Array.from(bookmarks).forEach(function (bookmark) {
      ...
      const url = bookmark.dataset.url
        ? bookmark.dataset.url.toLowerCase()
        : '';
      ...
      if (
        !isFolder &&
        (text.includes(searchTerm) || url.includes(searchTerm))
      ) {
        ...
        Array.from(childBookmarks).forEach(function (childBookmark) {
          ...
          const childUrl = childBookmark.dataset.url
            ? childBookmark.dataset.url.toLowerCase()
            : '';
  ...
  function loadBookmarks() {
    if (!bookmarksAPI || !bookmarksAPI.getTree) {
      console.error(
        'Bookmarks API is unavailable. Please check permissions or context.'
      );
      return;
    }
    ...
    bookmarksAPI
      ...
      .then(bookmarkTreeNodes => {
        ...
        if (isFirefox) {
          const toolbarNode = bookmarkTreeNodes[0]?.children?.find(
            node => node.title === 'Bookmarks Toolbar'
          );
          const menuNode = bookmarkTreeNodes[0]?.children?.find(
            node => node.title === 'Bookmarks Menu'
          );
          const otherNode = bookmarkTreeNodes[0]?.children?.find(
          ...
          const mainBookmarks = bookmarkTreeNodes[0]?.children?.find(
            node => node.title === default_folder
          );
  ...
  function displayBookmarks(bookmarkNodes) {
    ...
    const folders = bookmarkNodes.filter(
    ...
    for (let node of sortedNodes) {
      ...
      if (node.children && node.children.length > 0) {
        ...
        const folderIcon = document
          .getElementById('folderIconTemplate')
          .cloneNode(true);
        ...
        deleteButton.addEventListener('click', async function (event) {
          ...
          const confirmMessage = (
            translations[currentLanguage]?.deleteBookmark ||
            translations['en'].deleteBookmark
  ...
  saveBookmarkChanges.onclick = function () {
    ...
    if (isFirefox) {
      bookmarksAPI
        .update(currentBookmarkId, updatedData)
        .then(() => {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  bookmarksCheckbox.addEventListener('change', async function () {
    ...
    if (bookmarksPermission !== undefined) {
      ...
      await alertPrompt(
        translations[currentLanguage]?.UnsupportedBrowser ||
