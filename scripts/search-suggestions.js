...
const languageCode = (localStorage.getItem('selectedLanguage') || 'en').slice(
  0,
  2
);
            ...
            resultItem.onclick = () => {
              if (selectedOption === 'engine0') {
                try {
                  ...
                  var fallbackUrl =
                    searchEngines.engine1 + encodeURIComponent(suggestion);
                  window.location.href = fallbackUrl;
                }
              } else {
                var resultlink =
                  searchEngines[selectedOption] +
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  async function showProxyDisclaimer() {
    const message =
      translations[currentLanguage]?.ProxyDisclaimer ||
      translations['en'].ProxyDisclaimer;
