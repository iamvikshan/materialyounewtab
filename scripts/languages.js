...
function applyLanguage(lang) {
  ...
  if (translations[lang]) {
    const placeholder =
      translations[lang]?.userText || translations['en'].userText;
  ...
  if (menuCloseButton) {
    const hoverText =
      translations[lang]?.menuCloseText || translations['en'].menuCloseText;
  ...
  if (menuCont) {
    ...
    let widthh =
      window.innerWidth / parseInt(menuWidths[lang] || menuWidths['en']);
    if (window.innerWidth < 476) {
      let menuStyle =
        document.getElementById('menuStyle') || document.createElement('style');
  ...
  if (lang === 'vi') {
    ...
    root.style.setProperty(
      '--main-font-family',
      `"Be Vietnam Pro", ${commonFontStack}`
    );
  } else if (lang === 'ur') {
    loadFont(
    ...
    root.style.setProperty(
      '--main-font-family',
      `"Noto Sans Arabic", ${commonFontStack}`
    );
