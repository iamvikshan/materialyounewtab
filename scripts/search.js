...
const searchEnginesContainer = document.querySelector(
  '.searchEnginesContainer'
);
let activeSearchMode =
  localStorage.getItem('activeSearchMode') || 'search-with';

searchWith.addEventListener('click', function (event) {
  activeSearchMode =
    activeSearchMode === 'search-with' ? 'search-on' : 'search-with';
...
function toggleSearchEngines(category) {
  ...
  const checkeditem =
    localStorage.getItem(`selectedSearchEngine-${category}`) ||
    defaultItems[category];
  const searchModeName =
    category === 'search-with' ? 'searchWithHint' : 'searchOnHint';
  searchWith.innerText =
    translations[currentLanguage][searchModeName] ||
    translations['en'][searchModeName];
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  searchDropdowns.forEach(element => {
    element.addEventListener('click', () => {
      ...
      localStorage.setItem(
        `selectedSearchEngine-${radioButton.parentElement.dataset.category}`,
        radioButton.value
      );
      localStorage.setItem(
        `activeSearchMode`,
        radioButton.parentElement.dataset.category
      );
    });
  });
    ...
    if (searchTerm !== '') {
      if (selectedOption === 'engine0') {
        try {
          ...
          var fallbackUrl =
            searchEngines.engine1 + encodeURIComponent(searchTerm);
  ...
  let activeSearchMode =
    localStorage.getItem('activeSearchMode') || 'search-with';
  const storedSearchEngine = localStorage.getItem(
    `selectedSearchEngine-${activeSearchMode}`
  );
  ...
  searchEngineRadio.forEach(radio => {
    radio.addEventListener('change', () => {
      ...
      localStorage.setItem(
        `selectedSearchEngine-${selectedOption.parentElement.dataset.category}`,
        selectedOption.value
      );
      localStorage.setItem(
        `activeSearchMode`,
        selectedOption.parentElement.dataset.category
      );
    });
  });
