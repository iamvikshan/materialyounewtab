...
async function backupData() {
  try {
    ...
    await alertPrompt(
      (translations[currentLanguage]?.failedbackup ||
        translations['en'].failedbackup) + error.message
    );
  }
}
...
async function validateAndRestoreData(event) {
  ...
  reader.onload = async e => {
    try {
      ...
      await alertPrompt(
        translations[currentLanguage]?.restorecompleted ||
          translations['en'].restorecompleted
      );
      location.reload();
    } catch (error) {
      await alertPrompt(
        translations[currentLanguage]?.restorefailed ||
          translations['en'].restorefailed + error.message
      );
    }
  };
...
function isValidBackupFile(backup) {
  ...
  return !(
    ...
    typeof backup.indexedDB !== 'object'
  );
}
...
resetbtn.addEventListener('click', async () => {
  const confirmationMessage =
    translations[currentLanguage]?.confirmRestore ||
    translations['en'].confirmRestore;
