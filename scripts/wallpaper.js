...
async function applyRandomImage(showConfirmation = true) {
  if (
    showConfirmation &&
    !(await confirmPrompt(
      translations[currentLanguage]?.confirmWallpaper ||
        translations['en'].confirmWallpaper
    ))
  ) {
    return;
  }
...
function updateTextBackground(hasWallpaper) {
  ...
  [userText, date].forEach(element => {
    element.style.backgroundColor = hasWallpaper
  ...
  shortcuts.forEach(shortcut => {
    shortcut.style.backgroundColor = hasWallpaper
      ? 'var(--accentLightTint-blue)'
      : '';
...
document
  ...
  .addEventListener('click', async function () {
    try {
      ...
      if (!blob) {
        await alertPrompt(
          translations[currentLanguage]?.Nobackgroundset ||
            translations['en'].Nobackgroundset
        );
        return;
      }

      const confirmationMessage =
        translations[currentLanguage]?.clearbackgroundimage ||
        translations['en'].clearbackgroundimage;
